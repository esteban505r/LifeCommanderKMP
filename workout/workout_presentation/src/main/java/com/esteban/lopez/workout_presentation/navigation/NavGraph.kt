...
fun NavGraphBuilder.workoutGraph(
    ...
    composable("${Routes.WORKOUT_DAY_EXERCISES}/{workoutDayId}") {
        ...
        ExercisesDestination(
          ...
          onNewExerciseClick = {
            navController.navigate(Routes.NEW_EDIT_EXERCISE)
          },
            onExerciseClick = {
    ...
    composable(Routes.NEW_EDIT_EXERCISE){
        NewExerciseDestination(
            navController = navController
        )
    }

    composable("${Routes.NEW_EDIT_EXERCISE}/{exerciseToEditId}"){
        val exerciseToEditId = it.arguments?.getString("exerciseToEditId")
        NewExerciseDestination(
            navController = navController,
