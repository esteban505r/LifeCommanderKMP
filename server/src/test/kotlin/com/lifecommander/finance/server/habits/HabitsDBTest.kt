package lopez.esteban.com.habits
...
import com.esteban.ruano.utils.DateUtils.formatDate
...
class HabitsDBTest {
    ...
    fun setup() {
        ...
        transaction {
            val exerciseId = Habit.find { Habits.name eq "Daily Exercise" }.single().id
            val readBookId = Habit.find { Habits.name eq "Read a Book" }.single().id
    ...
    fun `should mark habit as complete`() {
        ...
        val tracks = transaction {
            HabitTrack.find { HabitTracks.habitId eq UUID.fromString(habit.id) }.toList()
        }
    ...
    fun `should uncomplete habit`() {
        ...
        habitService.unCompleteHabit(habit.id, formatDateTime(currentDateTime), 1)

        val habitUnDone = habitService.getByIdAndUserId(UUID.fromString(habit.id), 1, formatDate(currentDateTime.date))
    ...
    fun `should update habit name and reminders`() {
        ...
        val updatedHabit = habitService.getByIdAndUserId(UUID.fromString(habit.id), 1, formatDate(currentDateTime.date))
    ...
    fun `should soft delete habit`() {
        ...
        val deletedHabit = habitService.getByIdAndUserId(UUID.fromString(habit.id), 1, formatDate(currentDateTime.date))
