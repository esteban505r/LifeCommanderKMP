...
sealed class TaskIntent : UserIntent {
    ...
    data class AddTask(val name:String, val note:String?, val dueDate:String?, val scheduledDate:String?, val reminders:List<TaskReminder>, val onComplete: (Boolean) -> Unit) : TaskIntent()
    data class UpdateTask(val id: String, val task: com.esteban.ruano.tasks_domain.model.Task) : TaskIntent()
